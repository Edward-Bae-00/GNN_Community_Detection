...
class RecoveryBundleWriter:
    ...
    def _write_community(self, community):
        ...
        self._state["catalog_counts"] = {
            kind: self.catalog_store.count_active_records(
                kind, self._state["communities"]
            )
            for kind in ("nodes", "edges", "provenance")
        }
        self.checkpoint()
    ...
    def finalize_schema3(
        ...
        *,
        ...
        if generation_diagnostics is not None:
            detached_diagnostics = json.loads(_canonical_bytes(generation_diagnostics))
            narrative = (
                detached_diagnostics.get("narrative")
                if isinstance(detached_diagnostics, Mapping)
                else None
            )
            if isinstance(narrative, Mapping):
                compact_narrative = {}
                for field in (
                    ...
                    if field.endswith("error"):
                        if value is None:
                            continue
                        compact_narrative[field] = str(value)[:512]
                    elif isinstance(value, int) and not isinstance(value, bool):
                        compact_narrative[field] = value
                compact_diagnostics = {"narrative": compact_narrative}
        ...
        if compact_diagnostics is not None:
            core["generation_diagnostics"] = compact_diagnostics
