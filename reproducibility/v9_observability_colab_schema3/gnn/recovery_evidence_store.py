...
class RecoveryCatalogStore:
    ...
    def count_active_records(self, kind, community_keys):
        """Count the records ``iter_active_records`` would yield.

        Counting in SQLite avoids paging every row into Python and
        deserializing its canonical JSON purely to discard it, which is what a
        caller that only wants the size ends up doing.  ``records`` is keyed by
        ``(record_type, canonical_id)``, so ``canonical_json`` cannot vary for a
        given id and counting distinct ids matches the distinct rows the
        iterator yields.
        """
        self._validate_kind(kind)
        ...
        return int(
            self._connection.execute(
                """
                SELECT COUNT(DISTINCT records.canonical_id)
                FROM records
