...
# its own experimental defaults so ordinary runs from the handoff root are
# unaffected; the ``release`` subcommand replays the configuration recorded in
# the published checkpoint metadata.
PUBLISHED_RELEASE = {
    "seeds": (0, 1, 2),
    "epochs": 18,
    "train_bucket": "Q",
...
# that file is tracked in the source repository and pinned by its provenance
# tests, so a verification run must not overwrite it.
...
def _cli(argv=None):
    ...
    `PUBLISHED_RELEASE`, the configuration behind the published artifacts.
